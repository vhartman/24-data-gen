    ...
    for (const auto &r2 : robots) {
      ...
      for (uint i = 0; i < num_objects; ++i) {
        ...
        for (uint j = 0; j < max_attempts; ++j) {
          ...
          rai::Frame* obj1 = komo.pathConfig.getFrames(komo.pathConfig.frames.d1 * 1 + objID)(0);   
          obj1->setPose(C[obj]->getPose());

          rai::Frame* obj2 = komo.pathConfig.getFrames(komo.pathConfig.frames.d1 * 2 + objID)(0);
          obj2->setRelativePosition(obj1->getRelativePosition());
          obj2->setRelativeQuaternion(obj1->getRelativeQuaternion());

          rai::Frame* obj3 = komo.pathConfig.getFrames(komo.pathConfig.frames.d1 * 3 + objID)(0);
