...
void add_pick_constraints(KOMO &komo, const double start, const int end,
  ...
  komo.addObjective({start, start}, FS_positionDiff, {ee, obj},
                    OT_sos, {1e0});

  komo.addObjective({start, start}, FS_insideBox, {ee, obj},
                    OT_ineq, {5e1});
  ...
  if (dir == PickDirection::NegZ) {
    ...
    komo.addObjective({start, start}, FS_scalarProductXZ,
                      {obj, ee}, OT_sos, {1e1}, {-1.});
  } else if (dir == PickDirection::PosX) {
    komo.addObjective({start, start}, FS_scalarProductXZ,
                      {obj, ee}, OT_sos, {1e1}, {1.});
  } else if (dir == PickDirection::NegY) {
    komo.addObjective({start, start}, FS_scalarProductYZ,
                      {obj, ee}, OT_sos, {1e1}, {-1.});
  ...
  if (ee_type == EndEffectorType::two_finger) {
    if (obj_size(0) > obj_size(1)) {
      ...
      komo.addObjective({start, start}, FS_scalarProductXY,
                        {obj, ee}, OT_eq, {1e1}, {0.});
    } else {
      spdlog::info("Trying to grab along y-axis");
      komo.addObjective({start, start}, FS_scalarProductXX,
                        {obj, ee}, OT_eq, {1e1}, {0.});
    }
  }
}